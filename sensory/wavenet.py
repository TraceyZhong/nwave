...
class wavenetSC:
    ...
    def __init__(self, num_inputs, num_outputs, num_units, batch_size=1,
        eps = 5e-3, lmda=1e-2, lr_r=0.01, lr_W1=0.02, r1=3, r2=5, wi=1, we=5,
            sigmaE=3, leaky=0, target_sparsity=.10, **kwargs):
        self.D = num_inputs
        self.H1 = num_units
        self.K = num_outputs
        self.W1 = 0.3 * np.random.randn(self.D, self.H1)
        ...
        W = get_kernels(r1, r2, wi, we, sigmaE)
        E, I = get_W_matrix_from_kernel(num_units, r1, r2, wi, we,
            sigmaE, normalization=False)
        self.E = E
        self.cwsc = cwaveSC(
            num_inputs=3,
            num_units = self.side_len**2,
    ...
    def dictionary_local_gradient_step(self, words, activations):
        dW1 = words.T @ activations - words.T @ words @ self.W1
        self.W1 += self.lr_W1 * dW1
        self.normalize_cols()
    ...
    def train(self, X, gradient_steps=20000, initial_step=0):
        ...
        for i in tbar:
            ...
            current_sparsity = np.sum(np.abs(activations)>1e-5)/    np.prod(activations.shape)
            dlambda = 100*(current_sparsity - self.target_sparsity)
            self.cwsc.lmda = self.cwsc.lmda + .0001 * dlambda
            ...
            if i % 20 == 0:
                tbar.set_description("loss=%.3f sparsity=%2.2f%% steps=%d lmda=%.3f" % \
                    (l2l, 100*l0l, step, self.cwsc.lmda))
                tbar.refresh()
