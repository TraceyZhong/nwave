...
def getij(i, side_length):
    xi, yi = i//side_length, i%side_length
    return xi, yi

def get_W_matrix_from_kernel(n, r1, r2, wi=3, we=3, sigmaE = 3, normalization = False):
    assert r1 < r2
    side_length = int(np.sqrt(n))
    ...
    k_exc, k_inh = get_kernels(r1, r2, wi, we,sigmaE, normalization=normalization)
    for i in range(n):
        xi, yi = getij(i, side_length)
        for j in range(n):
            xj, yj = getij(j, side_length)
            dx, dy = abs(xi - xj), abs(yi - yj)
            if dx <= r1 and dy <= r1: 
                E[i,j] = k_exc[r1+dx, r1+dy]
            if (dx <= r2) and (dy <= r2):
                I[i,j] = - k_inh[r2+dx, r2+dy]
    return E, I

def get_kernels(r1, r2, wi=3, we=2, sigmaE = 2, normalization = True, **kwargs):
...
def write_arr_to_file(file, words):
    line = " ".join(map(str, words)) + "\n"
    with open(file, 'a') as f:
        f.write(line)
